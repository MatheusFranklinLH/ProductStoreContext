...
dotnet ef migrations add $Name --startup-project ../LeMat.Api/ $Params

echo "[BASH] Generating migration SQL script..."
Filename=$(TZ=GMT date +"%Y%m%d%H%M%S")\_$Name.sql
dotnet ef migrations script --output Migrations/$Filename --startup-project ../LeMat.Api/ -i $Params
...
>> dotnet ef database update --startup-project ../LeMat.Api/"
